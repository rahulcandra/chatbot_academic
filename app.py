...
/* ── SEMBUNYIKAN SPINNER & RUNNING INDICATOR ── */
...
div[class*="AppRunningIcon"]                 {{ display:none!important; }}
div[class*="appRunningIcon"]                 {{ display:none!important; }}
div[class*="runningIcon"]                    {{ display:none!important; }}
div[class*="RunningIcon"]                    {{ display:none!important; }}
...
/* ── SIDEBAR: tampil, bisa di-resize, tidak hilang saat drag ── */
...
/* Jangan sembunyikan sidebar lewat CSS — biarkan JS yang handle */
section[data-testid="stSidebar"][data-hidden="true"] {{
    display: none !important;
}}
/* Sembunyikan tombol bawaan Streamlit */
[data-testid="stSidebarCollapseButton"],
button[data-testid="collapsedControl"],
[data-testid="collapsedControl"] {{
    display: none !important;
}}
/* Tombol ☰ di pojok kiri atas (muncul saat sidebar tersembunyi) */
#custom-sidebar-toggle {{
    ...
    box-shadow: 0 2px 16px rgba(26,86,219,.6);
    transition: background .2s;
    line-height: 1;
}}
#custom-sidebar-toggle:hover {{ background: #0f2d6e; }}
/* Tombol ‹ fixed di tepi kiri (selalu terlihat saat sidebar terbuka) */
#custom-sidebar-close {{
    ...
    align-items: center;
    justify-content: center;
    box-shadow: 2px 0 12px rgba(26,86,219,.4);
    transition: background .2s, left .1s;
    font-weight: bold;
...
.stMarkdown, .stMarkdown * {{ color: {C['text']} !important; }}
...
[data-testid="stChatMessage"] a {{ color: {C['text']} !important; }}
[data-testid="stChatMessage"] code {{
...
[data-testid="stCaptionContainer"] p, .stApp small {{ color: {C['text_muted']} !important; }}
...
section[data-testid="stSidebar"] [data-white] {{ color: #ffffff !important; }}
...
div[data-testid="stChatInput"] button:hover {{
    background-color: {C['primary']} !important; opacity: 0.85 !important;
}}
...
button[data-testid="baseButton-primary"] {{
    background-color: {C['primary']} !important;
    border-color: {C['primary']} !important; color: white !important;
}}
...
div[data-testid="stAlert"] {{
    background: {C['surface2']} !important; border-color: {C['border']} !important;
}}
...
.header-title {{ font-size:22px; font-weight:800; margin:0; letter-spacing:-.5px; }}
.header-sub {{ font-size:12.5px; margin:3px 0 0; opacity:.85; }}
...
.krs-item {{
    ...
    border-radius:10px; background:{C['surface']}; border:1px solid {C['border']}; margin-bottom:7px;
}}
.krs-emoji-box {{
...
.landing-hero {{
    ...
    align-items:center; justify-content:center; text-align:center; padding:2.5rem 2rem 1rem;
}}
.landing-logo {{
...
.landing-tag {{
    ...
    padding:5px 12px; font-size:12px; font-weight:600; display:inline-block; margin:4px;
}}
.stat-card {{
    text-align:center; padding:20px 10px;
    background:{C['surface']}; border-radius:14px; border:1px solid {C['border']};
}}
.stat-val {{ font-size:32px; font-weight:800; color:{C['primary']}; font-family:'JetBrains Mono',monospace; }}
...
.sks-bar-bg {{ background:{C['border']}; border-radius:999px; height:8px; margin:8px 0 4px; overflow:hidden; }}
.sks-bar-fill {{ height:100%; border-radius:999px; transition:width .4s ease; }}
.fact-card {{
    background:{C['surface']}; border:1px solid {C['border']};
    border-left:4px solid {C['primary']}; border-radius:12px;
...
.tbadge {{ display:inline-block; border-radius:20px; padding:3px 10px; font-size:11px; font-weight:700; margin:2px 3px 2px 0; }}
.cmd-table {{
    background:{C['surface2']}; border:1px solid {C['border']};
...
(function() {
    ...
    function init() {
        ...
        // ── Tombol ☰ (buka sidebar, muncul saat sidebar hidden) ──
        var openBtn = document.getElementById('custom-sidebar-toggle');
        if (!openBtn) {
            openBtn = document.createElement('button');
            openBtn.id = 'custom-sidebar-toggle';
            openBtn.innerHTML = '&#9776;';
            openBtn.title = 'Buka Sidebar';
            document.body.appendChild(openBtn);
        }

        // ── Tombol ‹ (tutup sidebar, nempel di tepi kanan sidebar) ──
        var closeBtn = document.getElementById('custom-sidebar-close');
        if (!closeBtn) {
            closeBtn = document.createElement('button');
            closeBtn.id = 'custom-sidebar-close';
            closeBtn.innerHTML = '&#8249;';
            closeBtn.title = 'Sembunyikan Sidebar';
            document.body.appendChild(closeBtn);
        }

        // Posisikan tombol ‹ mengikuti lebar sidebar secara real-time
        function updateClosePos() {
            if (!sidebarVisible) return;
            var rect = sidebar.getBoundingClientRect();
            closeBtn.style.left = rect.right + 'px';
        }

        // Observer perubahan ukuran sidebar (termasuk saat di-drag)
        var ro = new ResizeObserver(function() {
            updateClosePos();
        });
        ro.observe(sidebar);

        // Update posisi saat window resize juga
        window.addEventListener('resize', updateClosePos);
        updateClosePos();
...
def render_catalog_card(key, data, in_krs):
    ...
    ts = (f'display:inline-block;border-radius:6px;padding:3px 8px;font-size:11px;'
    ...
    if not st.session_state.show_landing:
        ...
        <div data-white style="background:linear-gradient(135deg,#0f2d6e 0%,#1a56db 60%,#0ea5e9 100%);
            ...
            <div data-white style="font-size:10px;font-weight:700;letter-spacing:1.8px;
                        text-transform:uppercase;opacity:.75;margin-bottom:6px;color:#ffffff;">Total SKS Diambil</div>
            ...
            <div data-white style="margin:14px 0 12px;background:rgba(255,255,255,.18);
                ...
                <div data-white style="width:{pct}%;height:100%;border-radius:999px;background:rgba(255,255,255,.9);"></div>
            </div>
            <div data-white style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                <div data-white style="background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.18);
                    ...
                    <div data-white style="font-family:'JetBrains Mono',monospace;font-size:22px;font-weight:800;color:#ffffff;">{total_sks}</div>
                    <div data-white style="font-size:9px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;opacity:.7;color:#ffffff;">Diambil</div>
                </div>
                <div data-white style="background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.18);
                            border-radius:10px;padding:10px 12px;text-align:center;">
                    <div data-white style="font-family:'JetBrains Mono',monospace;font-size:22px;font-weight:800;color:#ffffff;">{max_sks - total_sks}</div>
                    <div data-white style="font-size:9px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;opacity:.7;color:#ffffff;">Tersisa</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if bot.cart:
            for c in bot.cart:
                ...
                <div class="krs-item">
                    ...
                    <div class="krs-text"><div class="krs-name">{nama}</div></div>
...
# ══════════════════════════════════════════════
        ...
        def chat_fragment():
            ...
            with chat_container:
                for msg in st.session_state.history:
                    avatar = (f"data:image/png;base64,{UPGRIS_LOGO_B64}"
                              if msg["role"] == "assistant" and UPGRIS_LOGO_B64
    ...
    with tab_catalog:
        ...
        with fc1:
            filter_kat = st.selectbox("Kategori", ["Semua", "Wajib", "Pilihan"], key="fkat")
        with fc2:
            filter_sem = st.selectbox("Semester", ["Semua","1","2","3","4","5","6","7","8"], key="fsem")
        with fc3:
            search_q = st.text_input("Cari Mata Kuliah atau Dosen", placeholder="Cari...", key="fsearch")
                ...
                for col_idx, (key, data) in enumerate(pair):
                    ...
                    with cols[col_idx]:
                        ...
                        if in_krs:
                            ...
                            if st.button("➕ Tambah ke KRS", key=f"add_{key}",
    ...
    with tab_schedule:
        ...
        if not bot.cart:
            ...
            pct = int(bot.total_sks() / bot.nlp.MAX_SKS * 100) if bot.nlp.MAX_SKS else 0
                ...
                for k, v in matkul_list:
                    ...
                    with badge_col1:
                        st.markdown(
                            ...
                            f'{stars} {diff_label}</span>', unsafe_allow_html=True)
                    with badge_col2:
                        st.markdown(
                            f'<span class="tbadge" style="background:{kat_bg};color:{kat_c};">'
                            f'{v["kategori"]}</span>', unsafe_allow_html=True)
  ...
  <div style="font-size:12px;color:{C['text_muted']};margin-top:4px;font-weight:600;">{lbl}</div>
    ...
    with tab_about:
        ...
        with col_a:
            ...
            st.markdown(
                "Chatbot ini menggunakan **Finite State Machine (FSM)** dengan **NLP Engine berbasis Regex** "
                "untuk membantu mahasiswa Teknik Informatika **UPGRIS** menyusun KRS."
            )
...
""")
        ...
        with col_b:
            ...
            rows = "".join(
                f'<div class="cmd-row"><span class="cmd-key">{cmd}</span>'
                f'<span class="cmd-desc">→ {desc}</span></div>'
